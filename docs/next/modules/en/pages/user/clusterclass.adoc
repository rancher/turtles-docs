...
+
Note that some variables are left to the user to substitute. +
Also beware that at the moment we only support initial provisioning with 1 control plane replica; this can be later increased, https://github.com/rancher/turtles/issues/1402[refer]. +
+
[source,yaml]
----
apiVersion: cluster.x-k8s.io/v1beta1
